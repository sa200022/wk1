...
public sealed class HealthServer : BackgroundService
{
    ...
    public HealthServer(ILogger<HealthServer> logger, int port = 6002)
    {
        _logger = logger;
        _port = port;
    }
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        try
        {
            ...
            _logger.LogInformation("Orchestrator health server listening on port {Port}", _port);
        }
        catch (Exception ex) when (ex is HttpListenerException or UnauthorizedAccessException or InvalidOperationException)
        {
            _logger.LogWarning(
                ex,
                "Orchestrator health server disabled (failed to bind to http://localhost:{Port}/health/).",
                _port);
            return;
        }
